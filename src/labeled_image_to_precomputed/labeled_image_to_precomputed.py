...
def labeled_image_to_precomputed(png_file, fullres_resolution=[1, 1, 1], target_dir="", destWidth=0, destHeight=0):
    ...
    for scale in fullres_info["scales"]:
        ...
        for ch in range(img_info.numChannels):
            ...
            spec = {
                "context": {"data_copy_concurrency": {"limit": 10}},
        ...
        if not (xRatio == 1 and yRatio == 1 and zRatio == 1):
            _resize_img = _resize_img.resize(
                desWidth=scale["size"][0],
        ...
        slices = [
            slice(idx * save_step, min(_resize_img.info.depth, save_step * (idx + 1)), 1)
            for idx in range((_resize_img.info.depth + save_step - 1) // save_step)
        ]
        ...
        for sl in slices:
            for ch in range(img_info.numChannels):
                ...
                reshape_data = np.reshape(
                    ravel_data, (ch_img_data.shape[-1], ch_img_data.shape[-2], ch_img_data.shape[-3]), order="F"
                )
                del ravel_data
                x_step = 1000
                ...
                for i in range(x_slice_num):
                    ...
                    arrs[ch][ts.d["channel"][0]][x_start:x_end, :, sl.start : sl.stop] = reshape_data[
                        x_start:x_end, :, sl.start : sl.stop
                    ]
...
if __name__ == "__main__":
    ...
    labeled_image_to_precomputed(
        args.image, fullres_resolution=args.r, target_dir=args.d, destWidth=args.width, destHeight=args.height
    )
