...
from convert_to_precomputed.convert import (
    LOG_FORMAT,
    build_ng_base_json,
...
def convert(
    image_path: Path = Argument(
        help="Image file or files directory",
        exists=True,
        dir_okay=True,
        file_okay=True,
        show_default=False,
    ...
    resume: bool = Option(
        help="Resume from output_directory/work_progress.json", default=True
    ),
    base_url: str = Option(
        help="Base url in base.json", default="http://10.11.40.170:2000"
    ),
    base_path: Path = Option(
        help="Base path, must be parent of output directory",
        default=Path("/zjbs-data/share"),
    ),
) -> None:
    logger.info(
...
def gen_spec(
    ...
    multiscale_metadata = build_multiscale_metadata_v2(
        image_info.dataTypeString(), image_info.numChannels
    )
    spec = ConvertSpec(
...
def convert_scale(
    spec_path: Path = Argument(
        help="Specification for converting image",
        exists=True,
        file_okay=True,
        dir_okay=False,
        show_default=False,
    ),
    scale_index: int = Argument(
        help="The scale to be converted in spec file", show_default=False
    ),
) -> None:
